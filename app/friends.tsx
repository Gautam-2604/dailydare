...
const FriendsPage = () => {
  ...
  const [receivedRequests] = useState<FriendRequest[]>([
    {
      id: 1,
      username: "alexcoder99",
      avatar: "AC",
      mutualFriends: 3,
      dateRequested: "2 hours ago",
      completedDares: 45,
      points: 2340,
    },
    {
      id: 2,
      username: "musicmaster_sarah",
      avatar: "MS",
      mutualFriends: 7,
  ...
  const getAvatarColor = (username: string) => {
    const colors = ['#4FFFB0', '#FFD700', '#FF6B6B', '#8A2BE2', '#00D4AA', '#FF8E8E'];
    const index = username.length % colors.length;
    return colors[index];
  };

  const FriendRequestCard: React.FC<{ request: FriendRequest; type: 'received' | 'sent' }> = ({ request, type }) => (
      ...
      >
        <View style={styles.requestHeader}>
          <View style={styles.userInfo}>
            <View style={[styles.avatar, { backgroundColor: getAvatarColor(request.username) }]}>
